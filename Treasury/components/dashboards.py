    ...
    dbc.Row([
            dbc.Col([
                dbc.Card([
                    ...
                    html.Label("Desbravadores",
                               style={"margin-top": "10px"}),
                    dcc.Dropdown(
                        id="dropdown-patfinder",
                        clearable=False,
                        style={"width": "100%"},
                        persistence=True,
                        persistence_type="session",
...
@app.callback([Output("dropdown-patfinder", "options"),
               Output("dropdown-patfinder", "value"),
               Output("p-patfinder-dashboards", "children")],
              Input("store-patfinder", "data"))
def populate_dropdownvalues(data):
    df = pd.DataFrame(data)
    valor = df['Valor'].sum()
    val = df.Desbravador.unique().tolist()

    return [([{"label": x, "value": x} for x in df.Desbravador.unique()]), val, f"R$ {valor}"]
...
@app.callback(
    ...
    [Input("store-expense", "data"),
     Input("store-receipt", "data")])
def balance_total(expense, receipt):
    df_expense = pd.DataFrame(expense)
    df_receipt = pd.DataFrame(receipt)

    valor = df_receipt['Valor'].sum() - df_expense['Valor'].sum()
...
@app.callback(
    ...
    [Input('store-expense', 'data'),
     ...
     Input("dropdown-patfinder", "value"),
     Input(ThemeChangerAIO.ids.radio("theme"), "value")])
def update_output(data_expense, data_receipt, expense, receipt, patfinder, theme):
    ...
    df_receipt_mes = df_rc.groupby("Mes")["Valor"].sum()
    df_expense_mes = df_ds.groupby("Mes")["Valor"].sum()
    df_balance_mes = df_receipt_mes - df_expense_mes
    ...
    df_rc = df_rc[df_rc['Desbravador'].isin(patfinder)]
    df_ds = df_ds[df_ds['Desbravador'].isin(patfinder)]

    fig = go.Figure()

    # fig.add_trace(go.Scatter(name='expense', x=df_ds['Data'], y=df_ds['Acumulo'], fill='tonexty', mode='lines'))
    fig.add_trace(go.Scatter(
        name='receipt', x=df_rc['Data'], y=df_rc['Acumulo'], fill='tonextx', mode='lines'))
...
def graph2_show(data_receipt, data_expense, receipt, expense, patfinder, start_date, end_date, theme):
    df_ds = pd.DataFrame(data_expense)
    df_rc = pd.DataFrame(data_receipt)

    dfs = [df_ds, df_rc]

    df_rc['Output'] = 'receipt'
    df_ds['Output'] = 'expense'
    ...
    df_end = df_end[df_end['Categoria'].isin(
        receipt) | df_end['Categoria'].isin(expense) | df_end['Desbravador'].isin(patfinder)]
...
def pie_receipt(data_receipt, receipt, theme):
    ...
    fig.update_layout(title={'text': "receipt"})
    fig.update_layout(margin=graph_margin, template=template_from_url(theme))
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)',
                      plot_bgcolor='rgba(0,0,0,0)')
...
def pie_expense(data_expense, expense, theme):
    ...
    fig = px.pie(df, values=df.Valor, names=df.Categoria, hole=.2)
    fig.update_layout(title={'text': "expense"})
